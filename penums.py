...
def print_to_string(enum, outfile):
    # duplicate values; some enums had new names added with the same value
    skiplist = (
...
{{"""
    print(fn.format(enum.name, enum.name), file=outfile)

    fmt = "\t\tcase {0} : return \"{0}\";"

    print("\tswitch(val) {", file=outfile)
    for name in [name for name in enum.names if name.startswith("NL80211_") and not name.endswith("_MAX") and name not in skiplist]:
        print(fmt.format(name, name), file=outfile)

    print("\t\tdefault: return \"unknown\";", file=outfile)
    print("\t}", file=outfile)  # switch close
    print("}", file=outfile) # function close
